...
class Gyro {
    ...
    public:
        ...
        float accel[3];          // Copy of ax, ay, az (0, 1, 2) for convenience
        float z_gforce;
        Gyro();
        uint8_t setupGyro();
...
uint8_t Gyro::setupGyro() {
  ...
  Serial.print("I AM ");  Serial.print(c, HEX);
  ...
  if (c == 0x68) {  
        ...
        if (SelfTest[0] < 1.0f && SelfTest[1] < 1.0f && SelfTest[2] < 1.0f && SelfTest[3] < 1.0f && SelfTest[4] < 1.0f && SelfTest[5] < 1.0f) {
            ...
            mpu.initMPU6050();  //TODO: Overload this to pass Gscale & Ascale
            ...
            return 0;   // Gyro initialisation SUCCESS!!
        } else {
  ...
  return -1; // Gyro Fail initialisation
}


void Gyro::ProcessGyroData() {

    if (mpu.readByte(MPU6050_ADDRESS, INT_STATUS) & 0x01) { // check if data ready and clear interrupt

        // Serial.println("####### mpuInterrupt fired! ###########");
    ...
    // @TODO: Verify if this change of parameter brings something
...
void Gyro::MadgwickQuaternionUpdate(float ax, float ay, float az, float gx, float gy, float gz)
{
    ...
    //  Serial.println("Inside Madgwick..........");
